...
- `AGENTS.md` — update conventions, the data-attributes table, and checklists to document any new render modes, data-attributes, or behaviors introduced. AGENTS.md is the authoritative source of conventions for this project and must stay current — update it whenever you introduce anything it should describe.
...
| `data-field-render="formatted" \| "formatted-multi-value" \| "formula" \| "calculated"` | Controls how a field renders and whether Cmd+K opens the link dialog. `formatted`, `formatted-multi-value`, and `formula` are used on hidden inputs (and dialog textareas). `calculated` is used on display-only `<span>` elements whose values are derived entirely in JS (e.g. stat modifiers); clicking them copies their text to the clipboard. | `formatted-multi-value` parses `---` (three or more dashes) as section separators. `calculated` spans have no paired input and are never directly edited. Values for the three input-based modes are driven by the `RENDER_MODES` registry in `app.js` — add a new mode there, not ad-hoc. |
| `data-layout="2col"` | Controls the visual arrangement of a field's rendered output. | Currently only used alongside `data-field-render="formatted-multi-value"` to display sections in two columns. Decoupled from render mode so layout can vary independently. |
